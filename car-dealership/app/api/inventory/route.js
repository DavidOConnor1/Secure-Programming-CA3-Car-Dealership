...
export async function GET(request) {
  try {
    ...
    if (
      ...
      if (
        filters.search.includes("' UNION") ||
        filters.search.includes("customers")
      ) {
        ...
        console.log(
          "🚨 POTENTIAL DATA LEAK: Customer data could be exposed via UNION attack"
        );
      }
    } else {
      // Normal safe search
    ...
    return Response.json({
      ...
      note:
        filters.search && filters.search.includes("' OR '1'='1")
          ? "Search returned all matching vehicles"
          : undefined,
    });
  } catch (error) {
    ...
    return Response.json(
      {
        ...
        message: isSqlError
          ...
          : error.message,
        hint: isSqlError
          ? "The search query may contain invalid SQL syntax"
          : undefined,
      },
      { status: 500 }
    );
